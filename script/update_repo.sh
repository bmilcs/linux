...
echo -e "${PUR}• ${BLU}bmilcs.git update ${NC}"
echo -e "${PUR}• ${BLU}YES REALLY UPDATING${NC}\n"
cd ~ && rm -rf ~/.bm_* ~/.dir_colors ~/.bm ~/.bmilcs ~/_bmilcs && git clone --quiet https://github.com/bmilcs/linux.git ~/.bmilcs && chmod -R +x ~/.bmilcs && (cd ~/.bmilcs/dotfiles ; ./install.sh) && source ~/.bashrc
echo -e "  ${GRN}[√] done.${NC}\n"
