...
(/usr/bin/git --git-dir=$HOME/docker/.git/ --work-tree=$HOME/docker commit -a -m "update" ; /usr/bin/git --git-dir=$HOME/docker/.git/ --work-tree=$HOME/docker push ; cd ~/docker ; docker-compose up -d --remove-orphans)
echo -e "  ${GRN}[√] done.${NC}\n"
